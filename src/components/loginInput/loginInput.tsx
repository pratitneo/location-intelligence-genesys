...
const LoginInput = ({
  ...
  return (
    <div className={`${inputCss["lip-input__wrap"]} ${inputValue ? inputCss["lip-input--filled"] : ""}`}>
      ...
      {icon === "passwordEye" ? (<span className={`${inputCss[`${customCls}`]} ${inputCss["lip-input__icon"]}`} onClick={updateVisibilityState}>{visible ? <IoEyeOffOutline size={20} /> : <IoEyeOutline size={20} />}</span>
      )
        : ''
      }
    </div >
